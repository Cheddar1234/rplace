...
app.put("/risuvai", function(req, res) {
    ...
    if(isNaN(Y)) {
        res.status(400);
        res.send("Y trqbva da e chislo");
        return;
    }
    if(Y < 0 || Y >= canvas_size) {
        res.status(400);
        res.send("Y trqbva da e mejdu 0 i " + canvas_size);
        return;
    }
    if(isNaN(R)) {
        res.status(400);
        res.send("R trqbva da e chislo");
        return;
    }
    if(R < 0 || R > 255) {
        res.status(400);
    ...
    if(isNaN(G)) {
        res.status(400);
        res.send("G trqbva da e chislo");
        return;
    }
    if(G < 0 || G > 255) {
        res.status(400);
        res.send("G trqbva da e mejdu 0 i 255");
        return;
    }
    if(isNaN(B)) {
        res.status(400);
        res.send("B trqbva da e chislo");
        return;
    }
    if(B < 0 || B > 255) {
        res.status(400);
        res.send("B trqbva da e mejdu 0 i 255");
        return;
    }
